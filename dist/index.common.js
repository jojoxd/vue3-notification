...
var script = vue.defineComponent({
    ...
    setup(__props, { emit }) {
        const props = __props;
        const { group, width, reverse, position, classes, animationType, animation, animationName, speed, cooldown, duration, delay, max, ignoreDuplicates, closeOnClick, pauseOnHover, } = vue.toRefs(props);
        const list = vue.ref([]);
        const velocity = vue.ref( /* TODO */);
        const timerControl = vue.ref(null);
        // @TODO: Refs
        const actualWidth = vue.computed(() => {
            return parse(width.value);
        });
        const isVA = vue.computed(() => {
        ...
        return (_ctx, _cache) => {
            return (vue.openBlock(), vue.createBlock("div", {
                ...
                (vue.openBlock(), vue.createBlock(vue.resolveDynamicComponent(vue.unref(componentName)), {
                    name: vue.unref(animationName),
                    onEnter: enter,
                    onLeave: leave,
                    onAfterLeave: clean
                }, {
                    default: vue.withCtx(() => [
                        (vue.openBlock(true), vue.createBlock(vue.Fragment, null, vue.renderList(vue.unref(active), (item) => {
                            return (vue.openBlock(), vue.createBlock("div", {
                                key: item.id,
                                ...
                                vue.renderSlot(_ctx.$slots, "body", {
                                    class: [vue.unref(classes), item.type],
                                    item: item,
                                    close: () => destroy(item)
                                }, () => [
